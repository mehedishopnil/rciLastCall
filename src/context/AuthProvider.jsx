...
const AuthProvider = ({ children }) => {
  ...
  const fetchResortData = async (page = 1, limit = 15) => {
    ...
    try {
      const response = await fetch(
        `https://rci-last-call-server.vercel.app/resorts?page=${page}&limit=${limit}`
      );
      if (!response.ok) {
        throw new Error(
  ...
  const fetchAllResorts = async () => {
    ...
    try {
      const response = await fetch(
        "https://rci-last-call-server.vercel.app/all-resorts"
      );
      if (!response.ok) {
        throw new Error(
    ...
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      if (currentUser) {
        ...
        const fetchUserData = async () => {
          try {
            const userDataResponse = await fetch(
              `https://rci-last-call-server.vercel.app/users?email=${currentUser.email}`
            );
            if (!userDataResponse.ok) {
              throw new Error("Failed to fetch user data from backend");
            }
